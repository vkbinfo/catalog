...
client_id = "575334475222-jp8jmuvk13k67aek18sig53dececue27" \
...
def gconnect():
    ...
    if not user_id:
        user_obj = User(username=login_session['username'],
...
def newCategory():
    ...
    if request.method == "POST":
        newCategoryObj = Category(name=request.form['name'],
...
def itemsInCategory(category_id):
    ...
    itemObjects = session.query(CategoryItem).filter_by(
        category_id=category_id).all()
...
def info_category_items_json(category_id):
    """list of items in given Category"""
    item_list = []
    ...
    return jsonify(category_items=item_list)


@app.route('/api/<int:category_id>/<int:item_id>')
def info_category_item(category_id, item_id):
    """Information about a singly category item"""
